...
function addEventListener() {
    ...
    window.addEventListener('message', (event) => {
        ...
        internalEventEmitter.emit(type, [foundWidgetId, foundIframe, data]);

        console.log('[Changelogify SDK] <-', type, data);
    }, false);

    // @ts-ignore
    internalEventEmitter.on('changelogify:internal:height', ([widgetId, iframe, data]) => {
        iframe.style.height = data.height + 'px';
    });

    // @ts-ignore
    internalEventEmitter.on('changelogify:internal:ready', ([widgetId, iframe, data]) => {
        ...
        globalEventEmitter.emit(WidgetEvent.WIDGET_READY, [widgetId, widgets.get(widgetId)]);
    })

    // @ts-ignore
    internalEventEmitter.on('changelogify:internal:read_article', ([widgetId, iframe, data]) => {
        const widget = widgets.get(widgetId) as AnyWidget;
